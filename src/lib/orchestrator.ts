...
/**
 * Fingerprint of the parts of an agent that live in the frozen system prompt.
 *
 * Claude Code bakes the system prompt into a session at creation and ignores a
 * changed `--append-system-prompt` on `--resume`. When this fingerprint stops
 * matching the stored one, the turn starts a fresh session so a persona edit
 * actually takes effect.
 */
function personaHash(agent: Agent): string {
  const source = [
    agent.name,
    agent.instructions ?? "",
...
/**
 * Prefix the context pack with the project brief and what this channel is for.
 *
 * These few lines are the cheapest steering available: they scope a turn
...
function runBrief(project: Project, channel: Channel, pack: string): string {
  ...
  const projectLines = [`## PROJECT ${project.name}`];
  if (project.description?.trim()) {
    projectLines.push(`Objective: ${project.description.trim()}`);
  }
  if (project.instructions?.trim()) {
    projectLines.push(project.instructions.trim());
  }
  sections.push(projectLines.join("\n"));

  const channelLines = [`## CHANNEL #${channel.name} (lane: ${channel.lane})`];
  if (channel.topic?.trim()) channelLines.push(channel.topic.trim());
...
interface LiveRun {
  ...
  lane: string;
  runRecordId: string;
  /** Persona the session was created with; stored on completion. */
  personaHash: string;
  buffer: string;
...
/**
 * The Claude session to resume for this agent in this channel.
 *
 * Returns "" when there is none, or when the agent's persona changed since the
 * session was created — resuming then would keep serving the old persona.
 */
async function resumeIdFor(agent: Agent, channelId: string): Promise<string> {
  try {
    const row = await pb
      .collection("agent_sessions")
      .getFirstListItem<AgentSession>(
        `agent = "${agent.id}" && channel = "${channelId}"`,
      );
    if (row.persona_hash && row.persona_hash !== personaHash(agent)) return "";
...
export async function dispatchTurn(
  ...
  const brief = runBrief(project, channel, pack.text);
